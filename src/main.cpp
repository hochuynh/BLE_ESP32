...
std::string uint8_to_string(int x)
{
    std::ostringstream ss;
    ss << x;
    return ss.str();
...
void notifyCB(NimBLERemoteCharacteristic* pRemoteCharacteristic, uint8_t* pData, size_t length, bool isNotify){
    ...
    if (std::string(pRemoteCharacteristic->getRemoteService()->getUUID()).find("1809") != std::string::npos)
    {
	    ...
	    mantissa = ((uint8_t *)pData)[1] + (((uint8_t *)pData)[2] <<8) + (((uint8_t *)pData)[3] <<16) ;
    
	    exponent = ((uint8_t *)pData)[4];
...
bool connectToServer() {
    ...
    pSvc = pClient->getService(std::string("180D"));
    if(pSvc) {     /** make sure it's not null */
        pChr = pSvc->getCharacteristic(std::string("2A37"));
        ...
        if(pChr) {     /** make sure it's not null */
            ...
            if(pChr->canNotify()) {
                //if(!pChr->registerForNotify(notifyCB)) {
                 Serial.println("Heart Rate service 1111 found.");
    ...
    pSvc = pClient->getService(std::string("1809"));
    if(pSvc) {     /** make sure it's not null */
        pChr = pSvc->getCharacteristic(std::string("2A1C"));
